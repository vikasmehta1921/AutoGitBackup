...
On non-Windows platforms (Linux, macOS, Streamlit Cloud),
all functions return graceful "not available" responses.
...
def _not_windows_response(action: str = "Task Scheduler") -> Dict:
    """Return a friendly response when not on Windows."""
    return {
        "success": False,
        "exists": False,
        ...
        "last_result": "—",
        "raw": "",
        "message": f"{action} is only available on Windows. "
                   "On Linux/Cloud, use a cron job or GitHub Actions instead.",
    }
...
def create_task(task_name: str = None, backup_time: str = None) -> Dict:
    ...
    Only works on Windows. Returns a friendly message on other platforms.
    """
    if not IS_WINDOWS:
        return _not_windows_response("create_task")
    ...
    result = _run_schtasks([
        ...
        "/TR", action, "/SC", "DAILY",
        "/ST", backup_time, "/RL", "HIGHEST",
    ])

    if result.returncode == 0:
        logger.info("Task '%s' created successfully.", task_name)
        return {"success": True, "message": f"Task '{task_name}' scheduled daily at {backup_time}."}
    else:
...
def delete_task(task_name: str = None) -> Dict:
    """Remove the Task Scheduler task. Windows only."""
    if not IS_WINDOWS:
        return _not_windows_response("delete_task")
...
def get_task_status(task_name: str = None) -> Dict:
    """Query Task Scheduler for task status. Returns gracefully on non-Windows."""
    if not IS_WINDOWS:
        return _not_windows_response("get_task_status")
